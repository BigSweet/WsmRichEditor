...
//        // 检查是否删除了@Span的一部分
//        RichEditText.AtSpan[] atSpans = editable.getSpans(cursorPos, cursorPos, RichEditText.AtSpan.class);
//        for (RichEditText.AtSpan span : atSpans) {
//            int spanStart = editable.getSpanStart(span);
//            int spanEnd = editable.getSpanEnd(span);
//            editable.removeSpan(atSpans);
//            editable.delete(spanStart, spanEnd); // 删除占位符
//            mEditText.removeByUserId(span.getUserId());
//            mEditText.getNameList();
//            break;
//        }
    ...
    public void beforeTextChanged(CharSequence s, int start, int count, int after) {
        ...
        if (count == 1) {
            ...
            if (" ".equals(deleteSb)) {
                ...
                RichEditText.AtSpan[] atSpans = editable.getSpans(beforeStart, beforeStart, RichEditText.AtSpan.class);
                if (atSpans.length > 0) {
                    mEditText.removeByUserId(atSpans[0].getUserId());
                }
            }
        }
...
//                mEditText.removeByUserId(span.getUserId());
                ...
                delIndex = -1;
                try {
                    mEditText.getText().replace(position, position + length, "");
