...
//                }
                int offset1 = textView.getOffsetForPosition(event.getX(), event.getY());

                AtStyleSpan[] atSpans1 = spannable.getSpans(offset1, offset1, AtStyleSpan.class);
                if (atSpans1.length > 0) {
                    return true;
                }
                return false;
...
//                    super.onTouchEvent(textView, spannable, event);
                ...
                if (event.getAction() == MotionEvent.ACTION_UP) {
                    ...
                    AtStyleSpan[] atSpans = spannable.getSpans(offset, offset, AtStyleSpan.class);
                    if (atSpans.length > 0) {
