...
import com.yuruiyin.richeditor.span.AtStyleSpan;
...
//        mRichEditText.setOnTouchListener((v, event) -> {
//            if (event.getAction() == MotionEvent.ACTION_UP) {
//                // 获取点击的字符位置
//                int offset = mRichEditText.getOffsetForPosition(event.getX(), event.getY());
//                // 检查点击位置是否在某个 Span 的范围内
//                Editable editable = mRichEditText.getEditableText();
//                RichEditText.AtSpan[] atSpans = editable.getSpans(offset, offset, RichEditText.AtSpan.class);
//                if (atSpans.length > 0) {
////                    RichEditText.AtSpan span = atSpans[0];
////                    int spanEnd = editable.getSpanEnd(span);
////                    int spanEnd = ((Spannable) mRichEditText.getText()).getSpanEnd(span);
////                    mRichEditText.setSelection(editable.length()); // 光标移动至 Span 末尾
////                    mRichEditText.clearFocus(); // 光标移动至 Span 末尾
////                    mRichEditText.requestFocus(); // 光标移动至 Span 末尾
//                    return true;  // 表示消费了点击事件
//                }
//            }
//            return false;
//        });
    ...
    List<RichEditorBlock> getContent(String editTextContent) {
        List<RichEditorBlock> richEditorBlockList = new ArrayList<>();
//        Editable editableTmp = (Editable) editTextContent;
        Editable editableTmp = Editable.Factory.getInstance().newEditable(editTextContent);
...
//            int atIndex = editable.toString().indexOf("@");
//            if (atIndex > -1) {
//                //这一行有@
//                AtStyleSpan[] atSpans = editable.getSpans(atIndex, atIndex, AtStyleSpan.class);
//                if (atSpans.length > 0) {
////                // 说明当前段落是ImageSpan, 没有行内样式
//                    AtStyleSpan blockImageSpan = atSpans[0];
//                    IBlockImageSpanObtainObject spanObtainObject = blockImageSpan.getAtVm();
//                    richEditorBlockList.add(
//                            getRichEditorBlock(
//                                    spanObtainObject.getType(), null, spanObtainObject, null
//                            )
...
//            }
    ...
    private IInlineSpan getInlineStyleSpan(Class spanClazz) {
        ...
        if (AtStyleSpan.class == spanClazz) {
            return new AtStyleSpan();
    ...
    private Class getSpanClassFromType(@RichTypeEnum String type) {
        switch (type) {
            ...
            case RichTypeEnum.AT:
                return AtStyleSpan.class;
    ...
    public void insertAtUser(FriendBean bean) {
        String atText = "@" + bean.getNickname() + " ";
        SpannableStringBuilder ssb = new SpannableStringBuilder(atText);
        ...
        mRichEditText.setSelection(start + atText.length());
    }
